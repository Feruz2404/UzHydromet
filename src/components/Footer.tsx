...
export function Footer() {
  ...
  return (
    <footer className="relative bg-gradient-to-br from-brand-navy via-brand-deep to-brand-navy text-white overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14 grid md:grid-cols-12 gap-10">
        <div className="md:col-span-4">
          <div className="flex items-center gap-3">
            <span className="w-11 h-11 rounded-xl bg-white/10 ring-1 ring-white/15 flex items-center justify-center text-brand-sky">
              <CloudSun size={22} />
            </span>
            <div>
              <div className="font-display font-extrabold text-lg leading-tight">{t('brand.short')}</div>
              <div className="text-[11px] text-white/70">{t('brand.tagline')}</div>
            </div>
          </div>
      ...
      <div className="relative border-t border-white/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 text-xs text-white/60 flex flex-col md:flex-row md:items-center md:justify-between gap-2">
          ...
          <div className="text-white/40">{t('footer.admin')}: {t('footer.adminText')}</div>
        </div>
      </div>
    </footer>
  )
