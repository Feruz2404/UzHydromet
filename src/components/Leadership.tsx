  ...
  return (
    <section id="leadership" className="py-16 lg:py-24 bg-white">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div {...headerMotion} className="mb-10 max-w-2xl">
          ...
          {leaders.map((leader) => {
            ...
            return (
              <motion.article key={leader.id} {...cardMotion} className="rounded-3xl bg-white border border-slate-100 shadow-card hover:shadow-glow transition-all overflow-hidden">
                <div className="grid gap-6 p-6 lg:p-7 lg:grid-cols-12 lg:items-start">
                  <div className="lg:col-span-2 flex lg:block items-center gap-4">
                    {leader.image ? (
                      <img src={leader.image} alt={leader.name} className="h-20 w-20 lg:h-28 lg:w-28 rounded-2xl object-cover bg-brand-mist border border-slate-100" />
                    ) : (
                      <div aria-hidden="true" className="h-20 w-20 lg:h-28 lg:w-28 rounded-2xl bg-gradient-to-br from-brand-primary to-brand-deep text-white flex items-center justify-center font-display text-2xl lg:text-3xl font-extrabold tracking-tight shadow-card">
                        {initials}
                      </div>
                    )}
                    <div className="lg:hidden flex-1 min-w-0">
                      <h3 className="font-display text-lg font-extrabold text-brand-navy leading-tight break-words">{leader.name}</h3>
                  ...
                  <div className="lg:col-span-5">
                    <ul className="space-y-2.5 text-sm">
                      ...
                      <li className="flex items-start gap-2.5">
                        ...
                        <span className="text-brand-muted leading-snug">{t(leader.addressKey)}</span>
                      </li>
                      <li className="flex items-start gap-2.5">
